...
| **M24** | **M-MULTI: multiple crew per phase** (customer ask, Round 4) | Base | ✅ **Done 2026-07-08** (`phase_assignees` junction replaces the single-FK assignment; up to `max_assignees_per_phase` per org — default 10, config not code — enforced in the action + a DB trigger; status stays one-per-phase shared last-writer-wins, M18 log attributes the actor; co-assignees read each other's crew notes AND photos, edit own only; REPLICA IDENTITY FULL + published so unassign deletes live-refresh; legacy column dropped by a follow-up cleanup migration after validation). |
