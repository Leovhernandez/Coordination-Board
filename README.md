...
Post-v1 (Trinity-driven, see [`docs/ROADMAP-AND-PRICING.md`](./docs/ROADMAP-AND-PRICING.md)): M14 multi-seat orgs → M-VIS company-wide read-only visibility → M-DASH symmetric dashboard → M13 Spanish/English UI (display-only) → M17 phase notes → M18 activity log + blocker duration → M10 soft-delete/restore/purge → M22 photo uploads (Cloudflare R2; Base+Pro storage cap) → M21 preferred payment method (owner opt-in, per-participant).
